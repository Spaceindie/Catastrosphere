...
function timerCallback()
  ...
  if (sunStart) then  mark += 1 end
    ...
    if mark >= 5 then
      ...
      backgroundSprite:moveTo(backgroundSprite.x, backgroundSprite.y -4)
...
if pd.buttonIsPressed(pd.kButtonA) and (gameStart == false) then 
  ...
  pressStartUISprite: remove()
    ...
    sonido = pd.sound.sampleplayer.new("/sound/coin")
    sonido:play()

    sonido2 = pd.sound.sampleplayer.new("/sound/soundtrack")
    sonido2:play(0)
...
if (gameOver) then
  ...
  -- wait for game over message to appear 
...
--------------------------------
   ...
   if pd.buttonJustPressed(pd.kButtonA) then
      ...
      if  powerAttack [currentPowerAttack][1] == "windPower" then 
        ...
        playerSprite: moveWithCollisions(playerSprite.x + windSpeed, playerSprite.y - 20)
